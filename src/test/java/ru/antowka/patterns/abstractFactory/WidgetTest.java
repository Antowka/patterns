 ...
 *   2) Метод getFactoryByLocale(String locale) возвращает необходмую фабрику на основе локализации en/ru
 *   3) Метод showWidgets(String locale) из полученной фабрики обробатывает widget-ы и отображает, не заботясь о их локализации
 *   в этом и заключается основной смысл патерна "Абстрактная фабрика", мы незная конкретного продукта, а запрашиваем его у
 *   имеющейся фабрики в результате получаем продук в соотвествии фабрики
 */
